...
# Simple module-level cache to avoid reloading data on every single
...
def _current_files_state(data_dir: str) -> dict:
    """Return a mapping path -> mtime for all cleaned CSV files in data_dir."""
    ...
    for p in files:
        try:
            ...
            # if a file disappears between listing and stat, ignore it
            continue
    return state


def get_data_if_new(data_dir: str = "data") -> dict:
    """Return cached data, reloading via `load_and_prepare_all` only when files changed.
    ...
    try:
        ...
        cur = {}
    if _CACHED_DATA is None or _CACHED_FILES_STATE != cur:
...
def make_app(data_dir: str = "data"):
    ...
    def create_layout():
        # ensure we have current data (will reload only if files changed)
